...
export default {
    ...
    async firstCheckId(id) {
        ...
        const driver = await FinancialStatements.findByPk(freight.financial_statements_id);
    ...
    async update(user, body, id) {
        const [freight, driver] = await Promise.all([Freight.findByPk(id), Driver.findByPk(body.driver_id)]);
